...
export default function EveryDealAddDialog(props) {
  ...
  const handleSubmit = (event) => {
    ...
    if (info.action === 'add') {
      ...
      axios.post(`${PROXY_URL}/api/EveryDeal/update`, formdata,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token_info")}`,
  ...
  return (
    <Modal isOpen={info.open} onClose={handleClose} title={info.action === 'add' ? "Add EveryDealation" : "Edit EveryDealation"} >
