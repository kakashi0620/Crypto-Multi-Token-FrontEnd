...
export default function AllDealsAddDialog(props) {
  ...
  const handleSubmit = (event) => {
    ...
    if (info.action === 'add') {
      axios.post(PROXY_URL + '/api/AllDeals/', formdata,
        {
          headers: {
      ...
      axios.post(`${PROXY_URL}/api/AllDeals/update`, formdata,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token_info")}`,
  ...
  // }
  return (
    <Modal isOpen={info.open} onConfirm={handleClose} onClose={handleClose} title={info.action === 'add' ? "Add AllDealsation" : "Edit AllDealsation"} >
